...
export class VillageScene extends Phaser.Scene {
    ...
    drawVillage() {
        ...
        g.fillStyle(0x0A0B1E, 1);
        g.fillRect(0, 0, 1600, 200);
        g.fillStyle(0x0E1635, 1);
        g.fillRect(0, 100, 1600, 100);
        g.fillStyle(0x1A2A5A, 1);
        g.fillRect(0, 180, 1600, 80);
        g.fillStyle(0x2A3A6A, 1);
        g.fillRect(0, 220, 1600, 60);
        g.fillStyle(0x3A4A5A, 1);
        g.fillRect(0, 260, 1600, 60);
        // Horizon glow
        g.fillStyle(0x6A4A2A, 0.3);
        g.fillRect(0, 300, 1600, 40);
        // Stars
        ...
        g.fillRect(0, 540, 1600, 60); // grass
        g.fillStyle(0x254012, 1);
        g.fillRect(0, 540, 1600, 12); // bright edge
        g.fillStyle(0x3A5A1A, 1);
        g.fillRect(0, 538, 1600, 4); // top strip
        // ── Cobblestone path (center of village)
        g.fillStyle(0x5A5055, 1);
        g.fillRect(380, 545, 620, 20);
        ...
        for (let px = 386; px < 990; px += 30) {
            g.fillRect(px, 547, 6, 2);
        }
        // ── Background trees (behind houses)
        for (const [tx, ty] of [[50, 460], [220, 440], [620, 430], [950, 440], [1200, 445], [1450, 435], [1550, 460]]) {
            // Trunk
            g.fillStyle(0x3A2818, 1);
            g.fillRect(tx + 12, ty + 40, 16, 35);
            g.fillStyle(0x4A3422, 1);
            g.fillRect(tx + 14, ty + 40, 8, 35);
            // Foliage (3 layers)
            g.fillStyle(0x1A4410, 1);
            g.fillTriangle(tx - 5, ty + 45, tx + 20, ty, tx + 45, ty + 45);
            g.fillStyle(0x225518, 1);
            g.fillTriangle(tx, ty + 32, tx + 20, ty + 10, tx + 40, ty + 32);
            g.fillStyle(0x2E6820, 1);
            g.fillTriangle(tx + 4, ty + 20, tx + 20, ty + 8, tx + 36, ty + 20);
            // Highlight
            g.fillStyle(0x3A8028, 0.4);
            g.fillTriangle(tx + 14, ty + 12, tx + 20, ty + 8, tx + 26, ty + 18);
        }
        // ── Houses ─────────────────────────────────────────────────────────────
        const houses = [
        ...
        for (const h of houses) {
            ...
            g.fillRect(x + 4, wallY + 4, w, wallH);
            // Main wall
            g.fillStyle(wall, 1);
            g.fillRect(x, wallY, w, wallH);
            // Wall shading (right & bottom darker)
            g.fillStyle(0x000000, 0.15);
            g.fillRect(x + w - 10, wallY, 10, wallH);
            g.fillStyle(0xFFFFFF, 0.08);
            g.fillRect(x, wallY, 10, wallH);
            // Wall texture (horizontal lines)
            g.fillStyle(0x000000, 0.06);
            for (let wy = wallY + 12; wy < ground; wy += 12) {
                g.fillRect(x, wy, w, 1);
            }
            // Roof
            const roofW = w + 20;
            g.fillStyle(0x000000, 0.2);
            g.fillTriangle(x - 8, wallY + 3, x + w / 2 + 2, wallY - h.h * 0.45 + 3, x + roofW - 8, wallY + 3);
            g.fillStyle(roof, 1);
            g.fillTriangle(x - 10, wallY, x + w / 2, wallY - h.h * 0.45, x + roofW - 10, wallY);
            // Roof highlight
            g.fillStyle(0xFFFFFF, 0.12);
            g.fillTriangle(x - 10, wallY, x + w / 2, wallY - h.h * 0.45, x + Math.floor(w * 0.3), wallY - h.h * 0.1);
            // Roof edge
            g.fillStyle(0x000000, 0.3);
            g.fillRect(x - 10, wallY - 2, roofW, 3);
            // Door
            const doorX = x + Math.floor(w / 2) - 10;
            g.fillStyle(door, 1);
            g.fillRect(doorX, ground - 30, 20, 30);
            // Door arch top
            g.fillStyle(door, 1);
            g.fillEllipse(doorX + 10, ground - 30, 20, 12);
            // Door shine
            g.fillStyle(0xFFFFFF, 0.1);
            g.fillRect(doorX + 2, ground - 28, 5, 14);
            // Door knob
            g.fillStyle(0xCC9920, 1);
            g.fillCircle(doorX + 16, ground - 17, 2);
            // Windows
            const winY = wallY + 12;
            for (const winX of [x + 8, x + w - 26]) {
                g.fillStyle(0x223344, 1);
                g.fillRect(winX, winY, 18, 16);
                g.fillStyle(0x88AACC, 0.6);
                g.fillRect(winX + 1, winY + 1, 16, 14);
                // Window panes
                g.fillStyle(0x000000, 0.25);
                g.fillRect(winX + 8, winY, 2, 16);
                g.fillRect(winX, winY + 7, 18, 2);
                // Window frame
                g.fillStyle(0xDDBB88, 1);
                g.fillRect(winX - 2, winY - 2, 22, 2);
                g.fillRect(winX - 2, winY + 16, 22, 2);
                g.fillRect(winX - 2, winY, 2, 16);
                g.fillRect(winX + 18, winY, 2, 16);
                // Warm light inside
            ...
            g.fillRect(x + 6, ground - wallH + 30, 20, 5);
            g.fillStyle(0xFF4466, 1);
            g.fillCircle(x + 9, ground - wallH + 28, 3);
            g.fillStyle(0xFF8800, 1);
            g.fillCircle(x + 14, ground - wallH + 26, 3);
            g.fillStyle(0xFF4466, 1);
            g.fillCircle(x + 19, ground - wallH + 28, 3);
            g.fillStyle(0x22AA44, 1);
            g.fillRect(x + 10, ground - wallH + 28, 2, 5);
        }
        // ── Fence posts
        g.fillStyle(0xAA8855, 1);
        for (let fx = 120; fx < 420; fx += 28) {
            g.fillRect(fx, 530, 6, 20);
            g.fillRect(fx - 2, 528, 10, 4);
        }
        for (let fx = 950; fx < 1350; fx += 28) {
            g.fillRect(fx, 530, 6, 20);
            g.fillRect(fx - 2, 528, 10, 4);
        }
        // Fence rail
        g.fillStyle(0x997744, 1);
        g.fillRect(120, 532, 296, 3);
        g.fillRect(120, 542, 296, 2);
        g.fillRect(950, 532, 396, 3);
        g.fillRect(950, 542, 396, 2);
        // ── Well (center)
        g.fillStyle(0x888888, 1);
        g.fillEllipse(650, 545, 50, 20);
        g.fillStyle(0x666666, 1);
        g.fillRect(626, 530, 48, 17);
        g.fillStyle(0x888888, 1);
        g.fillEllipse(650, 530, 50, 18);
        g.fillStyle(0x444444, 1);
        g.fillEllipse(650, 530, 36, 12);
        g.fillStyle(0x1A2A3A, 1);
        g.fillEllipse(650, 530, 28, 8);
        // Well roof
        g.fillStyle(0x553322, 1);
        g.fillRect(630, 510, 6, 22);
        g.fillRect(666, 510, 6, 22);
        g.fillStyle(0x7A3322, 1);
        g.fillTriangle(620, 514, 650, 500, 680, 514);
        // Bucket
        g.fillStyle(0x8B4513, 1);
        g.fillRect(646, 518, 8, 10);
        g.fillStyle(0xCC8844, 1);
        g.fillRect(647, 519, 6, 8);
        // ── Sign post
        g.fillStyle(0x6B4422, 1);
        g.fillRect(490, 520, 6, 28);
        g.fillStyle(0xAA7744, 1);
        g.fillRect(464, 512, 56, 16);
        g.fillStyle(0x000000, 0.7);
        g.fillRect(465, 513, 54, 14);
        // Sign text hint (dots representing letters)
        g.fillStyle(0xDDBB66, 1);
        for (let sx = 470; sx < 515; sx += 5) {
            g.fillRect(sx, 517, 3, 2);
        }
        // ── Barrels
        for (const bx of [340, 360, 1020, 1040]) {
            g.fillStyle(0x8B5A2A, 1);
            g.fillRect(bx, 524, 16, 18);
            g.fillStyle(0xAA7744, 1);
            g.fillRect(bx + 1, 525, 14, 16);
            g.fillStyle(0x8B5A2A, 1);
            g.fillRect(bx, 527, 16, 2);
            g.fillRect(bx, 534, 16, 2);
            g.fillStyle(0xCC9966, 0.25);
            g.fillRect(bx + 2, 526, 4, 6);
        }
    }
    // ── Player ─────────────────────────────────────────────────────────────────
    setupPlayer() {
