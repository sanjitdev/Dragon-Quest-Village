...
export class BossScene extends Phaser.Scene {
    ...
    setupArena() {
        ...
        for (const [x, w] of [[200, 4], [700, 5], [1200, 4]]) {
            for (let i = 0; i < w; i++) {
                const t = midG.create(x + i * 32, 440, 'tile_ground');
                t.refreshBody();
            }
        }
    ...
    setupBoss() {
        ...
        this.boss.onSpawnMinion = (x, y) => {
            ...
            this.physics.add.overlap(this.player.sprite, minion.sprite, () => this.combatSystem.playerHitsEnemy(this.player, minion));
            EventBus.once(Events.ENEMY_DIED, (dead) => {
                if (dead === minion)
                    this.boss.onMinionDefeated();
            });
        };
...
//# sourceMappingURL=BossScene.js.map