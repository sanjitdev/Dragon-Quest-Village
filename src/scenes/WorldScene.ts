...
export class WorldScene extends Phaser.Scene {
  ...
  private drawCloud(g: Phaser.GameObjects.Graphics, scale: number): void {
    ...
    g.fillStyle(0x1A2A3A, 0.18); g.fillEllipse(35 * s, 22, 80 * s, 18);
    g.fillStyle(0x1E2E42, 0.30); g.fillEllipse(35 * s, 14, 90 * s, 30);
    g.fillStyle(0x263446, 0.42); g.fillEllipse(10 * s, 10, 46 * s, 28 * s);
    g.fillStyle(0x2C3C52, 0.48); g.fillEllipse(30 * s,  4, 45 * s, 30 * s);
    g.fillStyle(0x324456, 0.45); g.fillEllipse(52 * s,  2, 40 * s, 28 * s);
    g.fillStyle(0x2A3A4E, 0.42); g.fillEllipse(70 * s,  9, 36 * s, 22 * s);
    // Faint moonlit edges
    g.fillStyle(0x8899BB, 0.12); g.fillEllipse(33 * s,  2, 28 * s, 14 * s);
    g.fillStyle(0x7788AA, 0.10); g.fillEllipse(55 * s, -1, 22 * s, 10 * s);
  }

  private updateClouds(delta: number): void {
    if (this.cloudObjs.length === 0) return;
    ...
    for (const cloud of this.cloudObjs) {
      ...
      if (cloud.wx < -300) cloud.wx = worldW + Phaser.Math.Between(0, 500);
  ...
  private setupBackground(): void {
    ...
    if (zone === 'forest') {
      // ── Sky
      g.fillStyle(0x0A1A0A, 1); g.fillRect(0, 0, 6400, 220);
      g.fillStyle(0x081808, 1); g.fillRect(0, 180, 6400, 80);
      // Moon
      g.fillStyle(0xE8E8C8, 1); g.fillCircle(300, 80, 35);
      g.fillStyle(0xC8C8A8, 1); g.fillCircle(288, 72, 30);
      g.fillStyle(0x0A1A0A, 1); g.fillCircle(310, 66, 24); // crescent cutout
      // Stars
      ...
      for (let tx = 0; tx < 6400; tx += 160) {
        ...
        // Moss/lichen
        g.fillStyle(0x22551A, 0.5); g.fillRect(tx + 44, 490, 8, 20);
      }
      // Fireflies
      g.fillStyle(0xAAFF44, 0.6);
      for (const [fx, fy] of [[180, 400],[380, 360],[640, 420],[900, 380],[1200, 410],[1500, 370],[1800, 400]]) {
        g.fillCircle(fx, fy, 3); g.fillStyle(0xAAFF44, 0.2); g.fillCircle(fx, fy, 6);
      }
      // Ground
      g.fillStyle(0x0E1E08, 1); g.fillRect(0, 540, 6400, 60);
      g.fillStyle(0x163010, 1); g.fillRect(0, 538, 6400, 8);
      // Fog
      g.fillStyle(0x88AACC, 0.04);
      for (let fx = 0; fx < 6400; fx += 300) { g.fillEllipse(fx + 150, 540, 400, 60); }

    } else if (zone === 'cave') {
      // ── Cave
      // Deep cave ceiling
      g.fillStyle(0x080810, 1); g.fillRect(0, 0, 6400, 600);
      // Rock strata bands
      g.fillStyle(0x0D0D1A, 1); g.fillRect(0, 80, 6400, 30);
      g.fillStyle(0x121220, 1); g.fillRect(0, 160, 6400, 20);
      ...
      for (const [cx2, cy2, color] of [
        ...
        for (let r = 80; r >= 20; r -= 20) {
          g.fillStyle(color, (80 - r) / 200); g.fillCircle(cx2, cy2, r);
        }
      }
      // Stalactites (ceiling)
      ...
      for (const [kx, ky, kc] of [
        ...
        g.fillStyle(0x8899DD, 0.4); g.fillRect(kx + 4, ky - 18, 4, 10);
      }
      // Torches
      for (const [tx, ty] of [[300, 380],[700, 360],[1200, 390],[1700, 370]]) {
        g.fillStyle(0x663300, 1); g.fillRect(tx - 2, ty, 5, 20);
        g.fillStyle(0xFF6600, 0.9); g.fillEllipse(tx, ty - 4, 10, 14);
        g.fillStyle(0xFF9922, 0.7); g.fillEllipse(tx, ty - 8, 7, 10);
        g.fillStyle(0xFFFF44, 0.5); g.fillEllipse(tx, ty - 10, 4, 7);
        // Torch glow
        g.fillStyle(0xFF6600, 0.06); g.fillCircle(tx, ty - 4, 60);
      }
      // Ground
      g.fillStyle(0x0A0A16, 1); g.fillRect(0, 555, 6400, 45);
      g.fillStyle(0x151525, 1); g.fillRect(0, 552, 6400, 6);
      ...
      for (let bx = 0; bx < 6400; bx += 64) {
        g.fillRect(bx, 0, 1, 80); g.fillRect(bx + 32, 16, 1, 48);
      }
      for (let by2 = 0; by2 < 80; by2 += 32) { g.fillRect(0, by2, 6400, 1); }
      // Chains
      for (const cx3 of [200, 600, 1000, 1400, 1800, 2200, 2600]) {
        for (let cy3 = 0; cy3 < 80; cy3 += 10) {
          g.fillStyle(0x445566, 1); g.fillCircle(cx3, cy3, 4);
          g.fillStyle(0x334455, 1); g.fillCircle(cx3, cy3, 3);
        }
      }
      // Torches
      for (const [tx, ty] of [[150, 370],[500, 350],[900, 380],[1300, 360],[1700, 375]]) {
        g.fillStyle(0x441100, 1); g.fillRect(tx - 2, ty, 5, 20);
        g.fillStyle(0xFF4400, 0.9); g.fillEllipse(tx, ty - 4, 10, 14);
        g.fillStyle(0xFF8822, 0.7); g.fillEllipse(tx, ty - 8, 7, 10);
        g.fillStyle(0xFF4400, 0.05); g.fillCircle(tx, ty - 4, 50);
      }
      // Floor
      g.fillStyle(0x101018, 1); g.fillRect(0, 555, 6400, 45);
      g.fillStyle(0x1A1A28, 1); g.fillRect(0, 552, 6400, 5);
      // Floor cracks
      g.fillStyle(0x000000, 0.5);
      for (let fx = 50; fx < 6400; fx += 200) {
        g.fillRect(fx, 556, 30, 1); g.fillRect(fx + 15, 556, 1, 8);
      }
    }
  }
