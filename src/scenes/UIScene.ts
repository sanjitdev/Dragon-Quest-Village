...
export class UIScene extends Phaser.Scene {
  ...
  private showingInventory: boolean = false;
  private inventoryKey!: Phaser.Input.Keyboard.Key;
  private notificationBus: string[] = [];
  ...
  private setupHUD(): void {
    ...
    // Controls hint
    this.add.text(GameConfig.WIDTH / 2, GameConfig.HEIGHT - 12,
  ...
  private renderInventory(player: Player): void {
    ...
    while (this.inventoryPanel.length > 2) {
      const child = this.inventoryPanel.getAt(2) as Phaser.GameObjects.GameObject;
      child.destroy();
  ...
  private setupInput(): void {
    ...
    this.input.keyboard.on('keydown-I', () => {
      ...
      if (this.showingInventory && this.playerRef) this.renderInventory(this.playerRef);
    });
  }
  ...
  refresh(player: Player): void {
    ...
    if (this.showingInventory) this.renderInventory(player);
  }
