...
export class UIScene extends Phaser.Scene {
  ...
  private totemSlot!:     Phaser.GameObjects.Text;
  // ── Right panel ─────────────────────────────────────────────────────────
  ...
  private notifBg!:       Phaser.GameObjects.Rectangle;
  // ── Inventory ────────────────────────────────────────────────────────────
  ...
  private buildTopBar(): void {
    ...
    pg.fillStyle(0x1A4A9C, 1); pg.fillRect(16, 20, 20, 18);
    pg.fillStyle(0xB8C4D4, 1); pg.fillRect(17, 10, 18, 13);
    pg.fillStyle(HUD.BG,   1); pg.fillRect(17, 15, 18, 5); // visor
    pg.fillStyle(0x44AAFF, 0.7); pg.fillRect(18, 16, 6, 2); pg.fillRect(26, 16, 6, 2);
    pg.fillStyle(0xCC2200, 1); pg.fillRect(22, 7, 4, 5);
    ...
    cg.fillStyle(0xCC8800, 1); cg.fillCircle(W - 90, 16, 8);
    cg.fillStyle(0xFFCC00, 1); cg.fillCircle(W - 90, 15, 7);
    cg.fillStyle(0xFFEE44, 1); cg.fillCircle(W - 91, 14, 4);
    cg.fillStyle(0xAA6600, 1); cg.fillCircle(W - 90, 15, 2);
  ...
  private buildNotification(): void {
  ...
  private renderInventory(player: Player): void {
    ...
    while (this.invPanel.length > 4) {
      const child = this.invPanel.getAt(4) as Phaser.GameObjects.GameObject;
      child.destroy();
    ...
    const eqText = this.add.text(10, 248,
      `Weapon: ${wName}  ATK +${wAtk}    Totem: ${tName}`, {
  ...
  private toggleInventory(): void {
    ...
    if (this.showingInv && this.playerRef) this.renderInventory(this.playerRef);
  }
  ...
  private buildHintBar(): void {
    ...
    this.add.text(W / 2, H - 8,
      '[←→] Move  [↑/X] Jump  [Z] Attack  [E] Interact  [I] Inventory  [M] Map', {
        fontFamily: HUD.FONT, fontSize: '5px', color: HUD.TEXT_DIM,
      }
  ...
  private connectEvents(): void {
    ...
    EventBus.on(Events.PLAYER_LEVELED_UP, (lvl: unknown) => {
      this.showNotification(`✦ Level Up!  Now Lv.${lvl} ✦`);
    });
    EventBus.on(Events.QUEST_COMPLETED, (q: unknown) => {
      const quest = q as { title: string };
      this.showNotification(`Quest Done: ${quest.title}`);
    });
    EventBus.on(Events.ITEM_PICKED_UP, (item: unknown) => {
  ...
  refresh(player: Player): void {
    ...
    if (this.showingInv) this.renderInventory(player);
  }
  ...
  override update(): void { /* player ref updated via events */ }
}
