...
export class WorldMapScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(GameConfig.WIDTH / 2, GameConfig.HEIGHT - 18,
      '[M] or [ESC] to close', {
  ...
  private buildNodes(): void {
    ...
    for (const zone of GameConfig.ZONES) {
      const pos = positions[zone.id as ZoneId];
      this.nodes.push({
        id:             zone.id as ZoneId,
        name:           zone.name,
        x:              pos.x,
        y:              pos.y,
        unlocked:       this.mapData.unlockedZones.includes(zone.id as ZoneId),
        requiredLevel:  zone.requiredLevel,
        nextScene:      this.zoneToScene(zone.id as ZoneId),
      });
    }
  }

  private zoneToScene(id: ZoneId): string {
    switch (id) {
  ...
  private showNodeInfo(node: ZoneNode): void {
    this.clearNodeInfo();
  ...
  private clearNodeInfo(): void {
    this.infoBox?.destroy();
    this.infoBox = undefined;
  }

  private travelToZone(node: ZoneNode): void {
    ...
    if (node.id === 'village') return; // already there
